...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, lm) #:: xs => {
      val whereToGo = newNeighborsOnly(neighborsWithHistory(b, lm), explored + b)
      whereToGo ++ from(xs ++ whereToGo, explored + b)
    }
  }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter { case (b, ml) => done(b) }
